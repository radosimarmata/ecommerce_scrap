...
def generate_embedding(text):
  try:
    ...
    res = response.json()
    embedding = res["embeddings"][0]  # vector 384
    return embedding
...
# ------------------------------------------------------------
# get_or_create_category (fixed)
# - Uses INSERT ... ON CONFLICT DO NOTHING with UNIQUE(name, level, parent_id)
# - Returns id (existing or newly inserted)
# ------------------------------------------------------------
def get_or_create_category(name, url, level, parent_id=None):
  ...
  # Try to return existing first (explicitly handle NULL parent)
